...
import { FiSearch, FiGlobe, FiPhoneCall, FiMapPin, FiChevronLeft, FiChevronRight, FiBriefcase } from 'react-icons/fi';
...
export default function AdminCompaniesListPage() {
  ...
  const fetchCompanies = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        const overallTotal = response.data.overall_total_count || total;
  ...
  useEffect(() => {
    // If search is fully cleared, reset back to global list immediately
    if (!searchInput.trim()) {
      setPage(1);
      setSearch('');
      return;
      ...
      <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <h1 className="page-title" style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            <FiBriefcase style={{ color: 'var(--primary)' }} /> Companies Directory
          </h1>
        </div>
        <div style={{ background: 'var(--primary)', color: 'white', padding: '0.5rem 1rem', borderRadius: '2rem', fontSize: '0.85rem', fontWeight: '600' }}>
          Total Companies: {overallTotalCount}
        </div>
      </div>
        ...
        {loading && companies.length === 0 ? (
          <div style={{ padding: '4rem', textAlign: 'center', color: 'var(--secondary)' }}>
            <div className="animate-pulse" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem' }}>
              ...
              <span>Fetching optimized directory data...</span>
            </div>
          </div>
        ) : companies.length === 0 ? (
          <div style={{ padding: '4rem', textAlign: 'center', color: '#64748b' }}>
            <FiBriefcase size={48} style={{ color: '#cbd5e1', marginBottom: '1rem' }} />
            <h3 style={{ fontSize: '1.1rem', fontWeight: '600', color: '#475569' }}>No companies found</h3>
